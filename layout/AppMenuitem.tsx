...
const AppMenuitemHooks = ({ item, keyValue, setActiveMenu }: { item: any, keyValue: string, setActiveMenu: (key: string) => void }) => {
    const pathname = usePathname();
    const searchParams = useSearchParams();
    const onRouteChange = (url: string) => {
        if (item!.to && item!.to === url) {
            setActiveMenu(keyValue);
        }
    };
    useEffect(() => {
...
const AppMenuitem = (props: AppMenuItemProps) => {
    ...
    return (
        <Suspense fallback={null}>
            <AppMenuitemHooks item={item} keyValue={key} setActiveMenu={setActiveMenu} />
